...
class Server:
    ...
    def __update_blockchain(self):
        ...
        self.__first_transaction(self.__parent.tab_wallet.get_key_object().publickey().export_key('DER'))
    ...
    def start_mining(self):
        self.__mining = True
        self.__parent.tab_blockchain.logger.log('Minnage en cours')

    def stop_mining(self):
        self.__mining = False
        self.__parent.tab_blockchain.logger.log('Arrêt du minnage')
    ...
    def __first_transaction(self, public_key: bytes):
        if not self.__blockchain.is_public_key_in_blockchain(public_key):
            private_key = self.__parent.tab_wallet.get_key_object()
            self.__mining_thread = Mining(self.__blockchain, private_key, self.__host, self.__parent)
